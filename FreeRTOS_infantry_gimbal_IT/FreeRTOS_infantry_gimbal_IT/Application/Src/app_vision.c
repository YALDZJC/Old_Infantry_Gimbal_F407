...
bool AbsoluteFilte()
{
	if(fabs(robot_pitch) < 25 && fabs(robot_yaw) < 25)
		return true;
	else
		return false;
}
...
//	uint8_t rx_buf[vision_receive_data_size];
	...
	for( ;; )
	{
		...
		if (rx_buf[0] == 0x5B && rx_buf[vision_receive_data_size - 1] == 0x5D)
		{
			...
			robot_pitch = (int16_t)((rx_buf[11] - 48) * 1000 + (rx_buf[12] - 48) * 100 + (rx_buf[13] - 48) * 10 +
									  (rx_buf[14] - 48));
			robot_yaw = (int16_t)((rx_buf[16] - 48) * 1000 + (rx_buf[17] - 48) * 100 + (rx_buf[18] - 48) * 10 +
									(rx_buf[19] - 48));
			/* ÊÓ¾õÊ¶±ðÎ» */
			vision_ready = 1;
			/* ÊÓ¾õ¿ª»ðÎ» */
			...
			pitch_angle = (motor_info_list[1]->angle_change_sum / 22.7527777);
			yaw_angle = yaw_cumulative_change_angle;
			if(pitch_angle < -46)
				pitch_angle = -46;
			if(pitch_angle > 32)
				pitch_angle = 32;
			...
			{
				vision_struct.vision_pitch_increment_angle = (float)((armor_pitch - 1000) / 10.f);
				vision_struct.vision_yaw_increment_angle = (float)((armor_yaw - 1000) / 10.f);
			}		
			
			if (vision_mode == robot)
			{
				vision_struct.vision_pitch_increment_angle = (float)((robot_pitch - 1000) / 10.f);
				vision_struct.vision_yaw_increment_angle = (float)((robot_yaw - 1000) / 10.f);

				vision_fire_flag = 0;
			}
			
			if(AbsoluteFilte() == true)
			{
				vision_struct.vision_pitch_increment_angle = 0;
				vision_struct.vision_yaw_increment_angle = 0;
			}
			

			
			if( vision_enable == 1 && aim_star == true)
			{
				yaw_target = (-vision_struct.vision_yaw_increment_angle+ yaw_angle)*22.7527777;
				pitch_target = (-vision_struct.vision_pitch_increment_angle + pitch_angle)*22.7527777;
				
				send_tar_flag = 0;
			}
			
			if(armor_pitch == 0 || armor_yaw == 0 || robot_pitch == 0 || robot_yaw == 0)
			{
				vision_struct.vision_yaw_increment_angle = 0;
				vision_struct.vision_pitch_increment_angle = 0;
...
//			else
//			{
//				yaw_target = yaw_cumulative_change_angle;
